...
exports.getTotalCampaignData = async (req, res) => {
    try {
      ...
      const viewsByLocation = Locations.map((loc) => [loc.City, loc.Views]);

      console.log(viewsByLocation)
      ...
      const registeredUsersByLocation = await UserR.findAll({
        attributes: [
          'city',
          [sequelize.fn('count', sequelize.col('id')), 'registeredUsers']
        ],
        group: ['city']
      });
      ...
      const activeUsersByLocation = await UserR.findAll({
        attributes: [
          'city',
      ...
      const formattedData =   registeredUsersByLocation.map(regData => {
        const location = regData.location;
